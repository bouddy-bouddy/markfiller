/* Backend License Service - Protected logic */

export interface LicenseValidationResult {
  valid: boolean;
  message: string;
  teacherName?: string;
  expiresAt?: string;
  daysRemaining?: number;
  devicesUsed?: number;
  maxDevices?: number;
}

export interface DeviceInfo {
  userAgent: string;
  platform?: string;
  language?: string;
  timezone?: string;
  screenResolution?: string;
}

class LicenseService {
  private readonly LMS_BASE_URL: string;

  constructor() {
    this.LMS_BASE_URL = process.env.LMS_BASE_URL || "https://markfiller-lms.vercel.app";
  }

  /**
   * Activate license with the LMS
   */
  async activateLicense(
    licenseKey: string,
    deviceId: string,
    deviceInfo: DeviceInfo,
    clientIP?: string,
    teacherProfile?: {
      cin?: string;
      phone?: string;
      level?: "الإعدادي" | "الثانوي";
      subject?: string;
      classesCount?: number;
      testsPerTerm?: number;
    }
  ): Promise<LicenseValidationResult> {
    try {
      const response = await fetch(`${this.LMS_BASE_URL}/api/activate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          key: licenseKey.trim(),
          deviceId,
          userAgent: deviceInfo.userAgent,
          ip: clientIP,
          profile: teacherProfile,
        }),
      });

      const data = await response.json();

      if (!response.ok) {
        return {
          valid: false,
          message: this.getErrorMessage(data.error || "فشل في تفعيل الترخيص"),
        };
      }

      return {
        valid: true,
        message: "تم تفعيل الترخيص بنجاح",
        expiresAt: data.validUntil,
      };
    } catch (error) {
      console.error("License activation error:", error);
      return {
        valid: false,
        message: "خطأ في الاتصال بخادم التراخيص",
      };
    }
  }

  /**
   * Validate existing license
   */
  async validateLicense(
    licenseKey: string,
    deviceId: string,
    deviceInfo: DeviceInfo,
    clientIP?: string
  ): Promise<LicenseValidationResult> {
    try {
      const response = await fetch(`${this.LMS_BASE_URL}/api/licenses/validate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          key: licenseKey,
          deviceId,
          ip: clientIP,
          userAgent: deviceInfo.userAgent,
        }),
      });

      const data = await response.json();

      if (!response.ok) {
        return {
          valid: false,
          message: this.getErrorMessage(data.message || "الترخيص غير صالح"),
        };
      }

      return {
        valid: true,
        message: data.message || "الترخيص صالح",
        teacherName: data.teacherName,
        expiresAt: data.expiresAt,
        daysRemaining: data.daysRemaining,
        devicesUsed: data.devicesUsed,
        maxDevices: data.maxDevices,
      };
    } catch (error) {
      console.error("License validation error:", error);
      return {
        valid: false,
        message: "خطأ في التحقق من الترخيص",
      };
    }
  }

  /**
   * Track usage events
   */
  async trackUsage(licenseKey: string, deviceId: string, eventType: string, metadata?: any): Promise<void> {
    try {
      await fetch(`${this.LMS_BASE_URL}/api/usage`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          key: licenseKey,
          deviceId,
          eventType,
          metadata,
          timestamp: new Date().toISOString(),
        }),
      });
    } catch (error) {
      console.warn("Usage tracking failed:", error);
    }
  }

  /**
   * Convert error codes to user-friendly Arabic messages
   */
  private getErrorMessage(error: string): string {
    const errorMessages: Record<string, string> = {
      "Invalid key": "مفتاح الترخيص غير صحيح",
      "License expired": "انتهت صلاحية الترخيص",
      "License suspended": "تم تعليق الترخيص",
      "Device limit exceeded": "تم تجاوز الحد الأقصى للأجهزة المسموح بها",
      "Device not activated": "الجهاز غير مفعل",
      "Activation failed": "فشل في التفعيل",
    };

    return errorMessages[error] || error;
  }
}

export const licenseService = new LicenseService();
